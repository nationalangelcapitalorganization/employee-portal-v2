export const createArticle = (article) => {
  return (dispatch, getState, { getFirestore }) => {
    ...
    firestore.collection('articles').add({
      ...
      firestore.collection('departments').doc(article.department).update({
    ...
    const { id, prevDepartment, ...noIdArticle } = article
     ...
     firestore.collection('articles').doc(article.id).update({
      ...
      if (article.prevDepartment === noIdArticle.department) {
        ...
        firestore.collection('departments').doc(article.prevDepartment).update({
          departmentArticles: firestore.FieldValue.arrayRemove(article.id)
        })
        firestore.collection('departments').doc(noIdArticle.department).update({
